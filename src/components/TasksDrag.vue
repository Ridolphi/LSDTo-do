...
<template>
  <div class="grid">
      ...
      <div class="card mb-0 bg-red-200">
        ...
        >
          ...
          <div class="text-right mt-2">
            <button
              class="p-button p-button-danger p-button-rounded mr-2"
    ...
    <Dialog v-model:visible="editModal.value" modal>
      <div class="p-4 w">
        ...
        <div class="mb-4">
          <select
            v-model="editedTask.value.tags.department"
            class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          >
            <option :value="editedTask.value.tags.department" selected>
              {{ editedTask.value.tags.department }}
            </option>
        ...
        <div class="mb-4">
          <select
            v-model="editedTask.value.tags.status"
            class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          >
            <option :value="editedTask.value.tags.status" selected>
              {{ editedTask.value.tags.status }}
            </option>
            <option value="To Do" v-if="editedTask.value.tags.status !== 'To Do'">
              To Do
            </option>
            <option
              value="In Progress"
              v-if="editedTask.value.tags.status !== 'In Progress'"
            >
              In Progress
            </option>
            <option value="Done" v-if="editedTask.value.tags.status !== 'Done'">
              Done
            </option>
          </select>
        </div>
    ...
    <Dialog v-model:visible="deleteModal.value" modal>
      <div class="p-4 w">
        <h3 class="text-lg font-semibold mb-4 text-inline">
          Are you sure to delete the next task "{{ selectedTask.value?.text }}" ?
        </h3>
        <div class="text-center">
          <button
            @click="() => { deleteTask(selectedTask.value.id); closeDeleteModal(); }"
